...
export default function ForecastSnapshot({ data, onOpen }: { data: LiveData; onOpen: () => void }) {
  ...
  useEffect(() => {
    if (!settings) return;
    const first = window.setTimeout(() => void loadForecast(settings), 0);
    const interval = window.setInterval(() => void loadForecast(settings), 60 * 60 * 1000);
  ...
  const forecast = useMemo(() => {
    ...
    const hours = calculateForecast(settings, weatherHours(weather), weather.daily.sunrise[0], weather.daily.sunset[0], calibration);
    const now = new Date(data.fetchedAt).getTime();
    const current = hours.reduce((best, hour) => !best || Math.abs(new Date(hour.time).getTime() - now) < Math.abs(new Date(best.time).getTime() - now) ? hour : best, hours[0] ?? null);
    const next = hours.find((hour) => new Date(hour.time).getTime() > now + 30 * 60 * 1000) ?? null;
    return { current, next, hours };
  }, [calibration, data.fetchedAt, settings, weather]);
