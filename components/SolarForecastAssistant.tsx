...
export default function SolarForecastAssistant({ data, onClose }: { data: LiveData; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const refresh = () => {
      if (!cancelled) void fetchWeather(settings);
    };
    const first = window.setTimeout(refresh, 0);
    const interval = window.setInterval(refresh, HOUR_MS);
    return () => {
      cancelled = true;
  ...
  }, [learning.calibration, learning.cloudCoefficient, learning.temperatureCoefficient, settings, weather]);

  const referenceTime = new Date(data.fetchedAt).getTime();
  const currentForecast = useMemo(() => nearestHour(forecast, referenceTime), [forecast, referenceTime]);
